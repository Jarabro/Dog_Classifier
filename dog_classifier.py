...
new_samoyd_data = np.column_stack((new_dachshund_length_data, new_samoyd_height_data)) #사모예드의 길이 높이 2차원배열로 합치기

dachshund_label = np.zeros(len(new_dachshund_data)) #닥스훈트는 0으로표시
samoyd_label = np.ones(len(new_samoyd_data)) #사모예드는 1으로표시

unknown_dog_length = np.random.randint(1, 100, size=5) #개 5마리의 길이 5개 생성
unknown_dog_height = np.random.randint(1, 60, size=5) #개 5마리의 높이 5개 생성
